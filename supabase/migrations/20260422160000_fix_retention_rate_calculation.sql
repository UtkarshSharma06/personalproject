-- Fix: Update retention rate to use activity (updated_at) and add separate weekly/monthly intervals
-- Date: 2026-04-22
...
BEGIN
    ...
    SELECT 
        ...
        END INTO retention_rate_weekly
    ...
    SELECT 
        CASE 
            WHEN total_users = 0 THEN 0 
            ELSE ROUND((COUNT(*)::NUMERIC / total_users::NUMERIC) * 100, 1)
        END INTO retention_rate_monthly
    FROM public.profiles
    WHERE updated_at >= NOW() - INTERVAL '30 days';
    
    -- Top Exams from practice responses only
