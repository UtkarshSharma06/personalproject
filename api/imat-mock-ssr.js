...
/**
 * Renders a single IMAT mock session as a server-side HTML card.
...
function renderMockCard(session) {
  ...
  return `
    <div class="bg-white rounded-[2rem] p-8 flex flex-col md:flex-row items-center justify-between border border-slate-100 shadow-sm">
      ...
      <a href="${isPast ? `/mock-guidelines?session_id=${session.id}&exam_type=imat-prep` : `/waiting-room/${session.id}`}"
         class="mt-6 md:mt-0 px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold uppercase text-[10px] tracking-widest">
        ${isPast ? 'Review Paper' : 'Register Now'}
      </a>
    </div>`;
}

function buildMockListSchema(sessions) {
  return {
    "@context": "https://schema.org",
    "@type": "ItemList",
    "name": "IMAT Official Mock Tests 2026 | ItaloStudy",
    "url": "https://italostudy.com/imat-mock",
    "numberOfItems": sessions.length,
    "itemListElement": sessions.slice(0, 10).map((s, i) => ({
      "@type": "ListItem",
      "position": i + 1,
      "name": s.title,
      "description": `Official IMAT mock test scheduled for ${new Date(s.start_time).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}.`,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (sessions.length === 0) {
      ssrHtml = `
        <div class="py-24 text-center">
          <h3 class="text-2xl font-black text-slate-400 uppercase tracking-tighter">No IMAT Simulations Scheduled Yet</h3>
          <p class="text-slate-400 mt-2 font-medium">Check back soon for upcoming IMAT 2026 mock tests.</p>
        </div>`;
    } else {
    ...
    const templatePath = path.join(process.cwd(), 'public', 'imat-mock.html');
    ...
    if (sessions.length > 0) {
      const schema = JSON.stringify(buildMockListSchema(sessions), null, 2);
    ...
    console.error('[imat-mock-ssr] Error:', err);
    try {
      const templatePath = path.join(process.cwd(), 'public', 'imat-mock.html');
