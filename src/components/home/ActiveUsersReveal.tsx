...
const ActiveUsersReveal = () => {
  ...
  const revealValue = getField('reveal_bank_size', '012000');
  const digits = revealValue.split('');
    ...
    <div className="w-full flex flex-col items-center justify-center p-4 md:p-8 min-h-[250px] md:min-h-[400px]">
      ...
      <section className="reveal-container">
        <p className="reveal-text">
          <EditableText fieldKey="reveal_title" fallback="Question Bank Size" />
        </p>
        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 opacity-60">
          <EditableText fieldKey="reveal_hint" fallback="Move to Reveal" />
        </p>
        <ul className="reveal-code">
          {digits.map((digit, i) => (
            <li key={i} tabIndex={0} className="reveal-digit">
              <span>{digit}</span>
            </li>
          ))}
        </ul>
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-2">
          <EditableText fieldKey="reveal_footer" fallback="Verified Questions" />
        </p>
      </section>
    </div>
  );
