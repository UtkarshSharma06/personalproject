...
import seoData from '@/data/seo/seo-tr.json';
...
const AcademicBackground = memo(() => {
    return (
        <div className="fixed inset-0 overflow-hidden pointer-events-none select-none bg-white -z-10 transform-gpu">
            {/* Turkish flag inspired colors - Red accents - Lightened */}
            <div className="absolute top-[-10%] left-[-10%] w-[70%] h-[70%] bg-red-500/5 blur-[120px] rounded-full" />
            <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-red-600/5 blur-[120px] rounded-full" />

            <div className="absolute inset-0 opacity-[0.4]">
                <svg className="w-full h-full text-red-100" aria-hidden="true">
                    <pattern id="light-grid-tr" x="0" y="0" width="100" height="100" patternUnits="userSpaceOnUse">
                        <circle cx="2" cy="2" r="1" fill="currentColor" />
                    </pattern>
                    <rect width="100%" height="100%" fill="url(#light-grid-tr)" />
                </svg>
            </div>
            ...
            <div className="absolute inset-0 overflow-hidden">
                {[...Array(6)].map((_, i) => (
                    <div
                        key={i}
                        className="absolute w-1 h-1 bg-red-200 rounded-full blur-[1px] opacity-0 md:opacity-100"
                        style={{
                            top: `${(i * 153) % 100}%`,
                            left: `${(i * 277) % 100}%`
                        }}
...
export default function IndexTurkey() {
    ...
    const { getField } = usePageContent(isEditMode ? 'landing-tr' : '');

    useEffect(() => {
        i18n.changeLanguage('tr');
        if (isEditMode) setPageSlug('landing-tr');
    }, [i18n, isEditMode]);
    ...
    return (
        <LiveEditProvider slug="landing-tr" fieldPrefix="tr_">
            <div className="min-h-screen font-sans selection:bg-orange-100 selection:text-orange-900 overflow-x-hidden relative bg-white">
                <SEO
                    title={getField('tr_seo_title', seoData.title)}
                    description={getField('tr_seo_description', seoData.description)}
                    ...
                    locale="tr_TR"
                    canonicalUrl="https://italostudy.com/tr"
                ...
                <div className={cn(
                    ...
                    <Link to="/status" className="relative flex items-center gap-2 md:gap-4 group transition-all px-4 md:px-6 py-1.5 md:py-1 w-full justify-center">
                        <div className="flex items-center gap-1.5 md:gap-2 px-2 md:px-3 py-0.5 md:py-1 bg-white/40 backdrop-blur-md rounded-full border border-blue-200/50 shadow-[0_0_15px_rgba(59,130,246,0.1)] group-hover:scale-105 transition-transform duration-300 shrink-0">
                            ...
                            <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.15em] text-blue-700">CANLI</span>
                        </div>
                        <div className="flex items-center gap-1.5 md:gap-2 overflow-hidden">
                            ...
                            <span className="text-[10px] md:text-[12px] font-bold tracking-tight text-slate-700 group-hover:text-blue-600 transition-colors truncate">
                                Durum Merkezi: <span className="text-slate-500 font-medium italic hidden sm:inline">2026 yol haritasını ve özelliklerini keşfedin</span>
                                <span className="text-slate-500 font-medium italic sm:hidden">2026 Yol Haritası Canlı</span>
                            </span>
                        </div>
                    </Link>
                </div>
                    ...
                    <div className="container mx-auto flex items-center justify-between">
                        ...
                        <nav className={cn(
                            ...
                            <div className="flex items-center gap-10">
                                ...
                                >
                                    <a href="/exams" className="text-[12px] font-bold tracking-tight transition-colors text-slate-500 hover:text-red-600 flex items-center gap-1 py-1">
                                        {t('nav.exams')}
                                    ...
                                    {isSyllabusDropdownOpen && (
                                            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-48 p-2 rounded-2xl bg-white border border-slate-100 shadow-2xl z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                                                <a href="/imat" className="flex items-center justify-between px-4 py-2.5 rounded-xl text-[11px] font-bold text-slate-400 hover:text-red-600 hover:bg-slate-50 transition-all group">
                                                    {t('nav.exam_items.imat')}
                                                    <ChevronRight className="w-3 h-3 opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                                                </a>
                                                <a href="/cent-s" className="flex items-center justify-between px-4 py-2.5 rounded-xl text-[11px] font-bold text-slate-400 hover:text-red-600 hover:bg-slate-50 transition-all group">
                                ...
                                <a href="/resources" className="text-[12px] font-bold tracking-tight transition-colors text-slate-500 hover:text-red-600">
                                    {t('nav.resources')}
                                </a>
                                <a href="/pricing" className="text-[12px] font-bold tracking-tight transition-colors text-slate-500 hover:text-red-600">
                                ...
                                <a href="/blog" className="text-[12px] font-bold tracking-tight transition-colors text-slate-500 hover:text-red-600">
                                    {t('nav.blog')}
                                </a>
                                <a href="https://store.italostudy.com" target="_blank" rel="noopener noreferrer" className="text-[12px] font-bold tracking-tight transition-colors text-slate-500 hover:text-red-600">
                                    Mağaza
                                </a>
                                <a href="/contact" className="text-[12px] font-bold tracking-tight transition-colors text-slate-500 hover:text-red-600">
                                    {t('nav.contact')}
                                </a>
                            </div>
                        </nav>
                        ...
                        <div className="flex-1 flex items-center justify-end gap-2 md:gap-4">
                            {user ? (
                                <button
                                    ...
                                    className="flex bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl px-4 md:px-6 py-2 items-center gap-2 transition-colors text-xs md:text-sm"
                                >
                                ...
                                <button
                                    ...
                                    className="flex bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl px-4 md:px-6 items-center justify-center h-9 md:h-10 text-xs md:text-sm"
                                >
                                    {t('nav.login')}
                                </button>
                            )}
                    ...
                    {isMobileMenuOpen && (
                            <div className="absolute top-full left-0 right-0 mt-4 mx-4 p-8 rounded-[2rem] bg-white border border-slate-100 lg:hidden flex flex-col gap-6 shadow-2xl z-50 animate-in slide-in-from-top-2 fade-in duration-200">
                                ...
                                <div className="flex flex-col gap-4">
                                        <a href="/exams" className="text-lg font-black tracking-tight text-slate-900" onClick={() => setIsMobileMenuOpen(false)}>{t('nav.exams')}</a>
                                        <div className="grid grid-cols-2 gap-2 pl-4">
                                            <a href="/imat" className="text-sm font-bold text-slate-400 hover:text-red-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>{t('nav.exam_items.imat')}</a>
                                            <a href="/cent-s" className="text-sm font-bold text-slate-400 hover:text-red-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>{t('nav.exam_items.cents')}</a>
                                        </div>
                                    </div>
                                    <a href="/resources" className="text-lg font-black tracking-tight text-slate-700" onClick={() => setIsMobileMenuOpen(false)}>{t('nav.resources')}</a>
                                    ...
                                    <a href="/blog" className="text-lg font-black tracking-tight text-slate-700" onClick={() => setIsMobileMenuOpen(false)}>{t('nav.blog')}</a>
                                    <a href="https://store.italostudy.com" target="_blank" rel="noopener noreferrer" className="text-lg font-black tracking-tight text-slate-700" onClick={() => setIsMobileMenuOpen(false)}>Mağaza</a>
                                    <a href="/contact" className="text-lg font-black tracking-tight text-slate-700" onClick={() => setIsMobileMenuOpen(false)}>{t('nav.contact')}</a>
                                </div>
                            </div>
                        )}
                </header>
                ...
                <section className="py-12 md:py-16 bg-slate-50 border-t border-slate-100 flex flex-col items-center text-center px-6">
                    ...
                    <button 
                        ...
                        className="h-14 md:h-18 px-8 md:px-12 bg-red-600 text-white font-black text-xs md:text-base lg:text-lg rounded-full hover:bg-red-700 shadow-2xl shadow-red-200 transition-all group w-full max-w-sm md:w-auto flex items-center justify-center"
                    >
                        {t('common.start_free')}
