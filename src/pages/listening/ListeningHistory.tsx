            ...
            <div className="container mx-auto px-6 py-12 max-w-5xl">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                    <div className="space-y-4">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-50 text-purple-600 text-[10px] font-black uppercase tracking-widest border border-purple-100">
                            <Headphones className="w-3 h-3" /> IELTS Module
                        </div>
                        <h1 className="text-5xl font-black text-slate-900 dark:text-slate-100 tracking-tighter leading-tight">
                            Listening <span className="text-purple-600">Logs</span>
                        </h1>
                        <p className="text-lg text-slate-400 font-bold tracking-tight">Review your performance in Listening simulations.</p>
                    </div>
                    ...
                    <div className="flex items-center gap-4 bg-white dark:bg-card p-4 rounded-[2rem] border-2 border-slate-100 dark:border-border shadow-xl shadow-slate-200/50">
                        ...
                        <div className="text-center px-6">
                            <p className="text-[8px] font-black text-slate-300 uppercase tracking-widest mb-1">Avg Accuracy</p>
                ...
                <div className="grid gap-6">
                    {evaluations.map((evalItem) => (
                        <div
                            key={evalItem.id}
                            onClick={() => navigate(`/listening/results/${evalItem.id}`)}
                            className="bg-white dark:bg-card p-8 rounded-[2.5rem] border-2 border-slate-100 dark:border-border border-b-[6px] shadow-xl shadow-slate-200/50 hover:border-purple-300 hover:-translate-y-1 transition-all duration-300 cursor-pointer group"
                        >
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div className="flex items-center gap-6">
                                    <div className="w-16 h-16 rounded-[1.5rem] bg-slate-50 dark:bg-muted border border-slate-100 dark:border-border flex items-center justify-center text-3xl group-hover:bg-purple-600 group-hover:text-white transition-all duration-500 shadow-inner">
                                        🎧
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-3 mb-2">
                                            <h3 className="font-black text-slate-900 dark:text-slate-100 tracking-tight leading-none uppercase text-base">IELTS Listening Mission</h3>
                                ...
                                <div className="flex items-center gap-8">
                                    <div className="bg-slate-50/50 dark:bg-muted/50 px-8 py-4 rounded-[1.5rem] border border-slate-50 dark:border-border text-center min-w-[120px]">
                                        <p className="text-[8px] font-black text-slate-300 uppercase tracking-widest mb-1.5">Accuracy</p>
                    ...
                    {evaluations.length === 0 && (
                        <div className="text-center py-32 bg-white dark:bg-card rounded-[4rem] border-2 border-slate-100 dark:border-border border-b-[10px] shadow-2xl shadow-slate-200/50">
                            ...
                            <h3 className="text-3xl font-black text-slate-900 dark:text-slate-100 mb-3 tracking-tight">No Logs Found</h3>
                            <p className="text-slate-400 font-bold mb-10 max-w-xs mx-auto text-lg leading-relaxed">Listen carefully and complete your first mission to track your accuracy here.</p>
                            <Button
